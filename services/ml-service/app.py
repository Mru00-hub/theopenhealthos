...
def train_pipeline():
    ...
    try:
        ...
        requests.post(f"{REGISTRY_URL}/models/register", json=reg_payload)
        
        # Step C: Validate (Call Validation Gateway)
        val_res = requests.post(f"{VALIDATION_URL}/validate", json={'model_id': model_name, 'metrics': metrics})
